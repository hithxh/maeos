"""Basilisk flight software models (FSW) are given in ``env.sim.fsw``.
...
actions are decorated with the :func:`~env.sim.fsw.action` decorator, which performs
...
Certain functions in the FSW models are decorated with the :func:`~env.utils.functional.aliveness_checker`
...
if TYPE_CHECKING:  # pragma: no cover
    ...
    from env.sim.dyn import DynamicsModel
...
def action(
    ...
    Action functions are typically called by :ref:`env.act` to task the satellite.
    """
    ...
    inner.__doc__ = "*Decorated with* :class:`~env.sim.fsw.action`\n\n" + str(
        func.__doc__
    )
...
class FSWModel(ABC):
    ...
    def _requires_dyn(cls) -> list[type["DynamicsModel"]]:
        """Define minimum :class:`~env.sim.dyn.DynamicsModel` for compatibility."""
        return []
    ...
    class LocPointTask(Task):
        ...
        def setup_instrument_controller(
            ...
            **kwargs,
            ...
            as set in :class:`~env.sim.dyn.ImagingDynModel.setup_imaging_target`).
