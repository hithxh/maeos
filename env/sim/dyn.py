"""Basilisk dynamics models are given in ``env.sim.dyn``.
...
:class:`~env.sats.Satellite`. Certain environment elements may require specific
dynamics models, such as :class:`~env.comm.LOSCommunication` requiring a dynamics
model that inherits from :class:`~env.sim.dyn.LOSCommDynModel` or :class:`~env.sats.ImagingSatellite`
requiring a dynamics model that inherits from :class:`~env.sim.dyn.ImagingDynModel`.

Setting Parameters
------------------

Customization of the dynamics model parameters is achieved through the ``sat_args``
dictionary passed to the :class:`~env.sats.Satellite` constructor. This dictionary is
...
via the :class:`~env.obs.SatProperties` observation.


Aliveness Checking
------------------

Certain functions in the dynamics model are decorated with the :func:`~env.utils.functional.aliveness_checker`
...
from env.sim import world
from env.utils import actuator_primitives as aP
from env.utils.attitude import random_tumble
from env.utils.functional import (
    aliveness_checker,
    check_aliveness_checkers,
    default_args,
)
from env.utils.orbital import random_orbit

if TYPE_CHECKING:  # pragma: no cover
    from env.sats import Satellite
    from env.sim import Simulator
    from env.sim.world import WorldModel
...
class DynamicsModel(ABC):
    ...
    def _requires_world(cls) -> list[type["WorldModel"]]:
        """Define minimum :class:`~env.sim.world.WorldModel` for compatibility."""
        return []
    ...
    def reset_for_action(self) -> None:
        """Reset whenever a flight software :class:`~env.sim.fsw.action` is called."""
        pass
...
class BasicDynamicsModel(DynamicsModel):
    ...
    def setup_gravity_bodies(self) -> None:
        """Set up gravitational bodies from the :class:`~env.sim.world.WorldModel` to included in the simulation."""
    ...
    @default_args(
        batteryStorageCapacity=800.0 * 3600.0,
        storedCharge_Init=lambda: 800.0 * 3600.0 #np.random.uniform(30.0 * 3600.0, 70.0 * 3600.0),
    )
    def setup_battery(
        self,
...
class LOSCommDynModel(BasicDynamicsModel):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        Necessary for :class:`~env.comm.LOSCommunication` to function.
        """
        super().__init__(*args, **kwargs)
...
class ImagingDynModel(BasicDynamicsModel):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        This dynamics model is used with :class:`~env.sats.ImagingSatellite`. It
        provides the satellite with the ability to take images of a point target. To
        enable downlink, use :class:`GroundStationDynModel` and :class:`~env.sim.world.GroundStationWorldModel`.
        """
        super().__init__(*args, **kwargs)
...
class GroundStationDynModel(ImagingDynModel):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        This model enables the use of ground stations defined in :class:`~env.sim.world.GroundStationWorldModel`
        for data downlink.
        """
        super().__init__(*args, **kwargs)
