"""Observations are found at ``env.obs``.
...
:class:`~env.env.scenario.satellites.Satellite` subclass. For example:
...
:class:`~env.env.scenario.satellites.Satellite`. The default is ``np.ndarray``, but
...
from env.obs.observations import (
    Eclipse,
    Observation,
