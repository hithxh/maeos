"""``env.utils.functional``: General utility functions."""
...
def aliveness_checker(func: Callable[..., bool]) -> Callable[..., bool]:
    ...
    inner.__doc__ = (
        "*Decorated with* :class:`~env.utils.functional.aliveness_checker`\n\n"
        + str(func.__doc__)
    )
...
class Resetable:
    ...
    These include :class:`~env.scene.Scenario`, :class:`~env.data.GlobalReward`,
    :class:`~env.comm.CommunicationMethod`, and :class:`~env.sats.Satellite`.

    The :class:`~env.GeneralSatelliteTasking.reset` method takes the following steps:
