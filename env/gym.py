...
from env.comm import CommunicationMethod, NoCommunication
from env.data import GlobalReward, NoReward
from env.sats import Satellite
from env.scene import Scenario
from env.sim import Simulator
from env.sim.world import WorldModel
from env.utils import logging_config
...
class GeneralSatelliteTasking(Env, Generic[SatObs, SatAct]):
    ...
    def __init__(
        ...
        Args:
            satellites: Satellite(s) to be simulated. See :ref:`env.sats`.
            scenario: Environment the satellite is acting in; contains information
                about targets, etc. See :ref:`env.scene`.
            rewarder: Handles recording and rewarding for data collection towards
                objectives. See :ref:`env.data`.
            communicator: Manages communication between satellites. See :ref:`env.comm`.
            ...
            world_args: Arguments for :class:`~env.sim.world.WorldModel` construction.
    ...
    def reset(
        ...
        Satellite and world arguments get randomized on reset, if :class:`~env.GeneralSatelliteTasking` ``.world_args``
        or :class:`~env.sats.Satellite` ``.sat_args`` includes randomization functions.

        Certain classes in ``env`` have a ``reset_pre_sim_init`` and/or ``reset_post_sim_init``
        method. These methods are respectively called before and after the new Basilisk
        :class:`~env.sim.Simulator` is created. These allow for reset actions that
    ...
    def _get_reward(self):
        ...
        # reward = list(self.reward_dict.values())
        for satellite in self.satellites:
            if not satellite.is_alive(log_failure=True):
                reward += self.failure_penalty
                # reward[self.satellites.index(satellite)] += self.failure_penalty
        return reward
