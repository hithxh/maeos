"""``env.sats`` defines satellite agents for the environments.
...
the observation and action spaces for the satellite. :ref:`env.act` and :ref:`env.obs`
...
specific dynamics or FSW model. See :ref:`env.sim.fsw` and :ref:`env.sim.dyn` for
...
These parameters are documented in :ref:`env.sim.fsw` and :ref:`env.sim.dyn`. To
...
* :class:`ImagingSatellite` - Extends :class:`AccessSatellite` to provide methods for
  interacting with :class:`env.scene.Target` objects.
"""

from env.sats.access_satellite import AccessSatellite, ImagingSatellite
from env.sats.satellite import Satellite
