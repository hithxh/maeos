...
from env.act.actions import select_action_builder
from env.obs.observations import ObservationBuilder
from env.sim import dyn, fsw
...
from env.utils.orbital import TrajectorySimulator

if TYPE_CHECKING:  # pragma: no cover
    from env.act import Action
    from env.data.base import DataStore
...
class Satellite(ABC, Resetable):
    ...
    def default_sat_args(cls, **kwargs) -> dict[str, Any]:
        """Compile default arguments for :class:`~env.sim.dyn.DynamicsModel` and :class:`~env.sim.fsw.FSWModel`, replacing those specified.
    ...
    def __init__(
        ...
        Args:
            ...
            sat_args: Arguments for :class:`~env.sim.dyn.DynamicsModel` and
                :class:`~env.sim.fsw.FSWModel` construction. Should be in the form of
            ...
            obs_type: Observation format for the satellite. The :class:`env.obs.observations.ObservationBuilder`
                will convert the observation to this format.
            variable_interval: Whether to stop the simulation at terminal events. If
                False, only the ``max_step_duration`` setting in :class:`~env.GeneralSatelliteTasking`
