...
from env.act.actions import Action, ActionBuilder

if TYPE_CHECKING:  # pragma: no cover
    from env.sats import Satellite
    from env.scene.targets import Target
...
class DiscreteFSWAction(DiscreteAction):
    def __init__(
        ...
        This action executes a function of a :class:`~env.env.simulation.fsw.FSWModel`
        ...
        Args:
            ...
            duration: Duration of the action in seconds. Defaults to a large value so that
                the :class:`~env.env.gym_env.GeneralSatelliteTasking` ``max_step_duration``
...
class Charge(DiscreteFSWAction):
    def __init__(self, name: Optional[str] = None, duration: Optional[float] = None):
        """Action to enter a sun-pointing charging mode (:class:`~env.env.simulation.fsw.BasicFSWModel.action_charge`).
...
class Drift(DiscreteFSWAction):
    def __init__(self, name: Optional[str] = None, duration: Optional[float] = None):
        """Action to disable all FSW tasks (:class:`~env.env.simulation.fsw.BasicFSWModel.action_drift`).
...
class Desat(DiscreteFSWAction):
    def __init__(self, name: Optional[str] = None, duration: Optional[float] = None):
        """Action to desaturate reaction wheels (:class:`~env.env.simulation.fsw.BasicFSWModel.action_desat`).
...
class Downlink(DiscreteFSWAction):
    def __init__(self, name: Optional[str] = None, duration: Optional[float] = None):
        """Action to transmit data from the data buffer (:class:`~env.env.simulation.fsw.ImagingFSWModel.action_downlink`).

        If not in range of a ground station (defined in
        :class:`~env.env.world.GroundStationWorldModel`), no data will
        be downlinked.
...
class Scan(DiscreteFSWAction):
    def __init__(self, name: Optional[str] = None, duration: Optional[float] = None):
        """Action to collect data from a :class:`~env.scene.UniformNadirScanning` (:class:`~env.sim.fsw.ContinuousImagingFSWModel.action_nadir_scan`).
...
class Image(DiscreteAction):
    def __init__(
        ...
        """Actions to image upcoming target (:class:`~env.env.simulation.fsw.ImagingFSWModel.action_image`).
        ...
        """
        from env.sats import ImagingSatellite
