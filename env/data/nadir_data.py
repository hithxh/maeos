...
from env.data.base import Data, DataStore, GlobalReward
...
class ScanningTimeReward(GlobalReward):
    ...
    def __init__(
        ...
        This class should be used with the :class:`~env.scene.UniformNadirScanning`
        scenario and a satellite with :class:`~env.sim.fsw.ContinuousImagingFSWModel`
        and the :class:`~env.act.Scan` action.

        Time is computed based on the amount of data in the satellite's buffer. In the
        basic configuration, this is the amount of time that the :class:`~env.act.Scan`
